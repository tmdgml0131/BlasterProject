...
void AHitScanWeapon::Fire(const FVector& HitTarget)
{
	...
	if (MuzzleFlashSocket)
	{
		...
		{
			if(HasAuthority() && !bUseServerSideRewind)		// NO ServerSideRewind
			{
				UGameplayStatics::ApplyDamage(BlasterCharacter, Damage, InstigatorController, this, UDamageType::StaticClass());
			}
			
			if(!HasAuthority() && bUseServerSideRewind)		// ServerSideRewind
			{
		...
		HandleCosmeticFeature(SocketTransform, FireHit);
	}
}

void AHitScanWeapon::HandleCosmeticFeature(FTransform SocketTransform, FHitResult FireHit)
{
	if (ImpactParticles)
	{
